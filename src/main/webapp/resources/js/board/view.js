...
window.onload = startupPage = async () => {
	...
	//선택된 항목 배경색 보여주기
	if(myLikeCheck.value == "Y") likeClick.style.backgroundColor="#ffc4d1";
	else if(myDislikeCheck.value == "Y") disLikeClick.style.backgroundColor="#ffc4d1";
...
//-- 좋아요 싫어요 처리 Start--//
	...
	likeClick.onclick = () => { 	
		...
		if(myLikeCheck == "Y" && myDislikeCheck == "N") {
			...
			document.querySelector('#likeClick').style.backgroundColor="white";
			...
			myDislikeCheck = "N";
			sendDataToServer(myLikeCheck,myDislikeCheck,checkCnt);
			document.querySelector('#likeClick').style.backgroundColor="#ffc4d1";
			document.querySelector('#disLikeClick').style.backgroundColor="white";
	...
	disLikeClick.onclick = () => {	
		if(myDislikeCheck == "Y" && myLikeCheck =="N") {
			...
			sendDataToServer(myLikeCheck,myDislikeCheck,checkCnt);
			document.querySelector('#likeClick').style.backgroundColor="white"; 
			document.querySelector('#disLikeClick').style.backgroundColor="#ffc4d1";
...
const replyList = (data) => {
	...
	for(const i in jsonInfo){
		...
		elm.setAttribute("style", "margin-left:80px; font-size: 12pt; text-align: left;"); //<div id='s3' style='font-size: 0.8em'></div>
		...
		result += "<div style='width:90%; height: auto; border-top: 1px solid gray; overflow: auto; text-align: left;' align='center'>";
...
window.addEventListener('load', () => {
	...
	function alert_invalid(){
		var msg;
		alert('수정 또는 삭제 권한이 없습니다.');	
	}

	/*******************이전글/다음글 이동*******************************/
	
	btn_preV.onclick = () => {		
		let pre_seq = document.querySelector('#btn_preV').value;
		//let preV_PageNum = document.querySelector('#preV_PageNum');
		let page = document.querySelector('#page').value;
		if(pre_seq == 0)		// 이전 글이 없는 경우
			alert('이전 글이 없습니다.');
		else document.location.href='/board/view?seqno=' + pre_seq + '&page=' + page.value + '&keyword=' + keyword.value;	//이전글(최근 글)로 이동
	}

	btn_nextV.onclick = () => {		
		let next_seq = document.querySelector('#btn_nextV').value;
		//let nextV_PageNum = document.querySelector('#nextV_PageNum').value;
		let page = document.querySelector('#page');
		if(next_seq == 0)		// 이전 글이 없는 경우
			alert('다음 글이 없습니다.');
		else document.location.href='/board/view?seqno=' + next_seq + '&page=' + page.value + '&keyword=' + keyword.value;	//이전글(최근 글)로 이동
	}
});